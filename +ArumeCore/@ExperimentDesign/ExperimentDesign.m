classdef ExperimentDesign < handle
    ...
    methods
        ...
        function initExisting(this, session, experimentData)
            this.Project = session.project;
            ...
            this.CurrentRun = experimentData.CurrentRun;
            this.PastRuns = experimentData.PastRuns;
            % TODO: maybe load more stuff.
        end
                    ...
                    switch( status )
                        ...
                        case RUNNING
                            ...
                            try
                                ...
                                if ( ~ (trialResult == Enum.trialResult.SOFT_ABORT) )
                                    ...
                                    if ( ~isempty(this.EyeTracker) )
                                        ...
                                        messageLeandro = { ''};
                                        a = randi(1);
                                        this.EyeTracker.ChangeStatusMessage('TRIAL N=%d Cond=%d NtoBreak=%d %s', trialnumber, currentCondition, this.ExperimentParams.Parameters.trialsBeforeBreak-trialsSinceBreak, messageLeandro{a});
                                    end
                                    %%-- Run the trial
                            ...
                            if ( trialResult == Enum.trialResult.CORRECT )
                                ...
                                switch(this.ExperimentParams.Parameters.trialAbortAction)
                                    case 'Repeat'
                                        % do nothing
                                    case 'Delay'
...
%                             elseif ( trialsSinceBreak >= this.ExperimentParams.Parameters.trialsBeforeBreak )
%                                 status = BREAK;
%                             elseif ( trialsSinceCalibration >= this.ExperimentParams.Parameters.trialsBeforeCalibration )
%                                 status = CALIBRATION;
%                             elseif ( trialsSinceDriftCorrection >= this.ExperimentParams.Parameters.trialsBeforeDriftCorrection )
...
%                             end
        ...
        function DisplayConditionMatrix(this)
            c = this.ExperimentParams.ConditionMatrix;
            for i=1:size(c,1)
                disp(c(i,:))
            end
        end
    ...
    methods (Sealed)
        ...
        function stats = GetStats(this)
            ...
            trialsPerSession = this.ExperimentParams.Parameters.trialsPerSession;
    ...
    methods(Access=protected)
        ...
        function variables = getVariablesCurrentCondition( this, currentCondition )
            ...
            conditionMatrix = this.ExperimentParams.ConditionMatrix;
            conditionVars = this.ExperimentParams.ConditionVars;
            ...
            for iVar=1:length(this.ExperimentParams.RandomVars)
                varName = this.ExperimentParams.RandomVars{iVar}.name;
                varType = this.ExperimentParams.RandomVars{iVar}.type;
                if ( isfield( this.ExperimentParams.RandomVars{iVar}, 'values' ) )
                    varValues = this.ExperimentParams.RandomVars{iVar}.values;
                end
                if ( isfield( this.ExperimentParams.RandomVars{iVar}, 'params' ) )
                    varParams = this.ExperimentParams.RandomVars{iVar}.params;
                end
    ...
    methods (Access=private)
        ...
        function setUpParameters(this)
            ...
            numberOfConditions = size(this.ExperimentParams.ConditionMatrix,1);
            ...
            this.ExperimentParams.Parameters = parameters;
        end
        
        %% setUpVariables
        function setUpVariables(this)
            [conditionVars randomVars] = this.getVariables();
            this.ExperimentParams.ConditionVars   = conditionVars;
            this.ExperimentParams.ConditionMatrix = this.getConditionMatrix( conditionVars );
            this.ExperimentParams.RandomVars      = randomVars;
        end
    ...
    methods ( Static = true )
        ...
        function experiment = New( session, experimentName )
            experiment = ArumeCore.ExperimentDesign.Create(experimentName);
            experiment.init(session);
        end
        
        function experiment = Load( session, experimentName, experimentData )
            experiment = ArumeCore.ExperimentDesign.Create(experimentName);
            experiment.initExisting(session, experimentData);
        end
